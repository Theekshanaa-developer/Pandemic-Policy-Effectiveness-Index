"""
4_deep_learning_forecasting/tcn_model.py
Temporal Convolutional Network model for forecasting.
"""
...
def train_tcn(country, df):
    ...
    model = Sequential([
        TCN(64, input_shape=(SEQ_LEN, X.shape[2])),
        Dense(1)
    ])
    ...
    out = pd.DataFrame({"y_true": y_test, "y_pred": preds_inv})
    out.to_csv(os.path.join(OUT_DIR, f"tcn_preds_{country}.csv"), index=False)

    print("Saved TCN predictions for", country)
    return model
...
if __name__ == "__main__":
    ...
    for country in ["India", "South Korea", "Australia"]:
        train_tcn(country, df)
